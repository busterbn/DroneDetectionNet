...
class StorageContext:
    ...
    def data_dir(self) -> Union[Path, str]:
        """Returns data directory path (local or GCS URI)."""
        if self.mode == "cloud":
            return settings.GCS_DATA_PATH  # Return gs:// URI for gcsfs
        return Path("data")

    @property
    def splits_dir(self) -> Union[Path, str]:
        """Returns splits directory path (local or GCS URI)."""
        if self.mode == "cloud":
            return f"{settings.GCS_DATA_PATH}/splits"  # Return gs:// URI for gcsfs
        return Path("data/splits")

    @property
    def models_dir(self) -> Union[Path, str]:
        """Returns models directory path (local or GCS URI)."""
        if self.mode == "cloud":
            return f"{settings.GCS_MODELS_BUCKET}/checkpoints"  # Return gs:// URI for gcsfs
        return Path("models")
    ...
    def _convert_to_onnx(self, model: torch.nn.Module, output_path: Union[str, Path]) -> None:
        ...
        """
        # Save original device to restore after conversion
    ...
    def load_state_dict(self, filename: str):
        ...
        if self.mode == "local":
            ...
            # Download from GCS using Google Cloud Storage client
            bucket_name = settings.GCS_MODELS_BUCKET.replace("gs://", "")
            ...
            logger.info(f"Downloaded checkpoint from GCS: {blob_path}")
    ...
    def load_onnx_path(self, filename: str = "model-latest.onnx") -> Union[Path, str]:
        ...
        if self.mode == "local":
            ...
            # Download from GCS to temp location using Google Cloud Storage client
            bucket_name = settings.GCS_MODELS_BUCKET.replace("gs://", "")
            blob_path = f"checkpoints/{filename}"
            ...
            temp_path = Path(tempfile.gettempdir()) / filename
            blob.download_to_filename(str(temp_path))
            logger.info(f"Downloaded model from GCS: {blob_path} -> {temp_path}")
