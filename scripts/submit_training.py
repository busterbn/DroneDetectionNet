...
def main(
    ...
    if sweep:
        ...
        # Use specified hyper_parameters config file
        container_args.append(f"hyper_parameters={config}")

        # Apply individual overrides if provided (override training.* which references hyper_parameters.*)
        if epochs is not None:
            container_args.append(f"training.epochs={epochs}")
        if batch_size is not None:
            container_args.append(f"training.batch_size={batch_size}")
        if lr is not None:
            container_args.append(f"training.lr={lr}")
        if profile:
            container_args.append("profile=true")
